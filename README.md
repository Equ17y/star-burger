...
YANDEX_GEOCODER_API_KEY=ваш_ключ_яндекс_геокодера
```
Вместо `ваш_ключ_яндекс_геокодера` подставьте реальный ключ, полученный в [Кабинете разработчика Яндекс](https://yandex.ru/dev/maps/geocoder/). Без этого ключа программа не сможет определять координаты адресов доставки.
